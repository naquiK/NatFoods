...
export default function OrderVibeCheck() {
    ...
    return (
      <div className="min-h-screen bg-stone-50 dark:bg-zinc-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => navigate(-1)} className="mt-4 px-3 py-2 rounded-lg border border-stone-300 dark:border-zinc-700">
            aight, go back
          </button>
        </div>
      </div>
  ...
  const currentStatusIndex = Math.max(0, statusVibes.findIndex(v => v.key === (orderDeets.status || "").toLowerCase()))
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-zinc-950 pt-24 pb-12">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold mb-1 text-stone-900 dark:text-white">Order #{orderDeets._id.slice(-8)}</h1>
            <p className="text-stone-600 dark:text-zinc-400">Placed on {new Date(orderDeets.createdAt).toLocaleDateString()}</p>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="px-3 py-2 rounded-lg border border-stone-300 dark:border-zinc-700 text-sm">
              Back
            </button>
            <button onClick={grabTheReceipts} className="px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 font-semibold text-sm">
              Download Invoice
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {orderDeets.items?.map((it) => {
              ...
              return (
                <div key={pid} className="flex items-center gap-4 p-4 bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-2xl">
                  ...
                  <div className="flex-grow">
                    <Link to={`/products/${pid}`} className="font-semibold text-stone-800 dark:text-white hover:underline">
                      {p.name || it.name}
                    </Link>
          ...
          <div className="lg:col-span-1 space-y-6">
                ...
                {statusVibes.map((vibe, idx) => {
                  ...
                  return (
                    <li key={vibe.name} className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isActive ? "bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400" : "bg-stone-100 dark:bg-zinc-800 text-stone-500 dark:text-zinc-500"}`}>
                        <vibe.icon className="w-5 h-5" />
                      </div>
                      <span className={`font-medium ${isActive ? "text-stone-800 dark:text-white" : "text-stone-500 dark:text-zinc-400"}`}>
                        {vibe.name}
                      </span>
                    </li>
                  )
            ...
            {/* Drop Zone */}
            <div className="p-6 bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-2xl">
              <h3 className="font-semibold mb-4 flex items-center gap-2"><MapPinIcon className="w-5 h-5" /> The Drop Zone</h3>
              <div className="text-sm text-stone-600 dark:text-zinc-300 space-y-1">
                ...
                <p>{dropLocation.city}, {dropLocation.postalCode}</p>
            ...
            <div className="p-6 bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-2xl">
              ...
              <div className="space-y-2 text-sm text-stone-600 dark:text-zinc-400">
                <div className="flex justify-between"><span>Subtotal</span><span>₹{Number(orderDeets.itemsPrice).toFixed(2)}</span></div>
                <div className="flex justify-between"><span>Shipping</span><span>₹{Number(orderDeets.shippingPrice).toFixed(2)}</span></div>
                <div className="flex justify-between"><span>Taxes</span><span>₹{Number(orderDeets.taxPrice).toFixed(2)}</span></div>
                <div className="flex justify-between text-base font-bold text-stone-900 dark:text-white pt-2 border-t border-stone-200 dark:border-zinc-700 mt-2">
                  <span>Total</span><span>₹{Number(orderDeets.totalAmount).toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {otherVibes?.length > 0 && (
          <section className="mt-12">
            <h2 className="text-xl font-semibold mb-4 text-stone-900 dark:text-white">you might also vibe with these 👀</h2>
